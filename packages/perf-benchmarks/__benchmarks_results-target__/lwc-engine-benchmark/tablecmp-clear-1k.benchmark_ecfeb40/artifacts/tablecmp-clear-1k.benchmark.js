(function () {
  ...
  */
  ...
  function allocateVMChildrenHook(vnode) {
  ...
  const globalRegisteredNames = new Set();
  /**
  ...
  function defineUpgradableElement(tagName) {
  ...
  function registerTagName(tagName) {
  ...
  function isTagNameRegistered(tagName) {
    return globalRegisteredNames.has(tagName);
  }

  function isUpgradableElement(elm) {
  ...
  const CustomElementHook = {
    create: vnode => {
      ...
      linkNodeToShadow(vnode);
      fallbackElmHook(vnode); // If this `sel` is claimed by a custom element,
  ...
  function resetStyle(vm) {
    ...
    const {
      hostAttribute,
      styleElement
    } = context; // Remove the style attribute currently applied to the host element.
    ...
    context.hostAttribute = context.shadowAttribute = undefined; // removing style tag if present
  ...
  function applyStyle(vm, stylesheets, stylesheetTokens) {
    const {
      context,
      elm,
      cmpRoot
    ...
    context.shadowAttribute = shadowAttribute; // Caching style vnode so it can be removed when needed

    const styleElm = context.styleElement = evaluateCSS(vm, stylesheets, hostAttribute, shadowAttribute);
  ...
  function resetTemplate(vm) {
    ...
    for (let i = 0, len = children.length; i < len; i += 1) {
      ...
      if (!isNull(vnode)) {
        const {
          elm
        } = vnode; // the vnode must have elm defined, and must be connected

        cmpRoot.removeChild(elm);
      }
    }

    vm.children = EmptyArray; // intentionally leaving out any manually inserted node in the shadowRoot
  }
  ...
  function resetReactiveListeners(vm) {
    const {
      oar,
      tro
    } = vm; // Making sure that any observing record will not trigger the rehydration on this vm
  ...
  function rehydrate(vm) {
    ...
    if (isTrue$1(vm.isDirty) && isTrue$1(vm.isConnected)) {
  ...
  function resetShadowRootAfterError(vm) {
    ...
    for (let i = 0, len = children.length; i < len; i += 1) {
      ...
      if (!isNull(vnode)) {
        ...
        if (!isUndefined(elm)) {
          ...
          try {
            cmpRoot.removeChild(elm);
          } catch (e) {// ignore any error to complete the clean up
          }
        }
      }
    }
  ...
  function createElement(sel, options) {
    ...
    registerTagName(sel); // Create element with correct tagName
  ...
  */
  ...
  var _benchmarkTableComponentRow = registerComponent(TableComponentRow, {
    tmpl: _tmpl$1
  });
  ...
  function tmpl$1($api, $cmp, $slotset, $ctx) {
    ...
    return [api_element("section", {
      ...
      return api_custom_element("benchmark-table-component-row", _benchmarkTableComponentRow, {
  ...
  benchmark(`benchmark-table-component/clear/1k`, () => {
    ...
    let store;
    before(async () => {
      tableElement = createElement('benchmark-table-component', {
        is: Table
      });

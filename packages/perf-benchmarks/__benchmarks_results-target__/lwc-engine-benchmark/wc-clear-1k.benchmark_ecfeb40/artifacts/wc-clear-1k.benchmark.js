...
`;
  ...
  */
  ...
  function resetStyle(vm) {
    ...
    if (!isUndefined$1(hostAttribute)) {
      removeAttribute$1.call(elm, hostAttribute);
    } // Reset the scoping attributes associated to the context.


    context.hostAttribute = context.shadowAttribute = undefined; // removing style tag if present

    if (!isNull$1(styleElement) && !isUndefined$1(styleElement)) {
    ...
    runWithBoundaryProtection(vm, vm.owner, () => {
      ...
      tro.observe(() => {
        ...
        if (html !== cmpTemplate) {
          ...
          if (!isUndefined$1(stylesheetTokens) && !isUndefined$1(stylesheets) && stylesheets.length > 0) {
  ...
  function rehydrate(vm) {
    ...
    if (isTrue$1$1(vm.isDirty) && isTrue$1$1(vm.isConnected)) {
